...
## A limitation I found and couldn't fully resolve

While building a second `ExternalSecret` (in the `demo-app` namespace, pulling
the same `database/creds/readonly` role) I hit something worth documenting
honestly rather than hiding: credentials synced by ESO through this dynamic
secrets path consistently failed to authenticate against Postgres, even
though everything about the setup was correct.
...
That last test is the key one: the same token, hitting the same endpoint,
over the same network path, works when called directly — but consistently
fails when the credential comes from ESO's own sync. That isolates the gap
to something specific in how ESO's Vault-provider client requests or
processes responses from a **dynamic** secrets engine, as opposed to a
static KV read (which is the far more common and better-tested path for
this provider).

I didn't chase this further into ESO's source, partly because time was
better spent elsewhere, and partly because I'd already produced strong,
specific evidence of exactly where the boundary of the problem sits — which
is arguably more useful to document than a silent, unexplained fix would
have been. For the actual demonstration of bidirectional sync, I relied on
the static-secret `PushSecret` path instead (see below), which doesn't
touch this code path at all and works reliably.

**If I were to keep pushing on this**, my next steps would be: pin an
exact ESO patch version and check its changelog for dynamic-secrets fixes,
try the same setup against a real HashiCorp Vault instance instead of
OpenBao to see if the behavior is OpenBao-specific or a general ESO
Vault-provider issue, and open an issue upstream with this exact isolation
writeup if neither of those explains it.
